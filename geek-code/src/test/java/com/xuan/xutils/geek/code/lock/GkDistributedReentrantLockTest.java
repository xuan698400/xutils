 ...
 * @since 2023/9/10
 */
public class GkDistributedReentrantLockTest {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        //2、构建一个可重入的锁
        GkDistributedReentrantLock lock = new GkDistributedReentrantLock(dataSource,
            "GkDistributedReentrantLockTest");
        ...
        // 主线程第1次获取锁，预期成功
        boolean success = lock.tryLock(Integer.MAX_VALUE);
        if (success) {
            log("主线程获取锁，成功【预期】");
        } else {
            log("主线程获取锁，失败【非预期】");
        ...
        //主线程第2次获取锁，可重入，预期成功
        success = lock.tryLock(Integer.MAX_VALUE);
        if (success) {
            log("主线程重入获取锁，成功【预期】");
        } else {
            log("主线程重入获取锁，失败【非预期】");
