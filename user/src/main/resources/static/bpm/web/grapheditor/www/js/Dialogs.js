...
ExportDialog.exportFile = function (editorUi, name, format, bg, s, b) {
    ...
    if (format == 'xml') {
        ...
        if (data.length < MAX_REQUEST_SIZE) {
            editorUi.hideDialog();
            open('/export?xml=' + encodeURIComponent(data) + '&filename=' + encodeURIComponent(filename));
        }
        else {
        ...
        //add by xuan
        var editorUi = editorUi;
        var data = mxUtils.getXml(graph.getSvg(bg, s, b));
        var filename = name;
        ...
        if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA) {
            ...
            //open('/exportImg?xml=' + encodeURIComponent(mxUtils.getXml(root)) + '&filename=' + encodeURIComponent(name));
        }
        else {
            mxUtils.alert(mxResources.get('drawingTooLarge'));
        }
